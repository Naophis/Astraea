...
const yamlStringify = (obj, indent = 0, set_comment) => {
  ...
  if (Array.isArray(obj)) {
    ...
    return obj.map(v => `${IND}- ${yamlStringify(v, indent + 1, set_comment).replace(/^\s+/, '')}`)
      .join('\n');
  } else {
    ...
    return keys.map(k => {
      ...
      if (v !== null && typeof v === 'object') {
        const nested = yamlStringify(v, indent + 1, set_comment);
        // ネストは改行＋インデント
        return `${IND}${key}:\n${nested}`;
      } else {
        if (set_comment && key == "fast") {
          return `${IND}${key}: ${yamlStringify(v, 0, set_comment)} # ${index++}`;
        }
        return `${IND}${key}: ${yamlStringify(v, 0, set_comment)}`;
      }
    }).join('\n');
  }
}
