...
void MainTask::load_hw_param() {
  ...
  param->angle_pid.b = getItem(angle_pid, "b")->valuedouble / 180 * M_PI;
  ...
  param->gyro_param.list_size = getItem(gyro_param, "list_size")->valueint;
  param->gyro_param.loop_size = getItem(gyro_param, "loop_size")->valueint;
...
void MainTask::load_offset_param() {
  ...
  param->dia45_offset_max_dist =
      getItem(root, "dia45_offset_max_dist")->valuedouble;
...
void MainTask::load_straight(
  ...
  if (sys.hf_cl == 0) {
    fileName = "/spiflash/vel_prof.hf";
  } else {
    fileName = "/spiflash/vel_prof.cl";
  }
  std::ifstream ifs(fileName);
  std::string str = "";
  std::string buf;
